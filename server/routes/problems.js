...
router.get("/", auth, async (req, res) => {
	try {
		...
		// Build filter
		// Show user's own problems OR approved global problems
		const filter = {
			$or: [
				{ userId: req.user.id }, // User's own problems (all statuses)
				{ isGlobal: true, approvalStatus: "approved" }, // Only approved global problems
			],
		};
	...
	async (req, res) => {
		try {
			...
			if (existingProblem) {
				return res
					.status(400)
					.json({ message: "Problem already exists in your list" });
			}
...
router.put("/:id", auth, async (req, res) => {
	try {
		...
		if (spaceComplexity !== undefined)
			problem.spaceComplexity = spaceComplexity;
		if (customTags)
			problem.customTags = customTags.map((tag) => tag.toLowerCase());
		...
		for (const problemData of problems) {
			try {
				...
				const problem = new Problem({
					...
					customTags:
						problemData.customTags?.map((tag) => tag.toLowerCase()) || [],
				});
	...
	try {
		...
		for (const problem of solvedProblems) {
			...
			if (!lastDate) {
				...
				const daysDiff = Math.floor(
					(new Date(lastDate) - new Date(solvedDate)) / (1000 * 60 * 60 * 24)
				);
		...
		if (solvedProblems.length > 0) {
			...
			const lastSolvedDate = new Date(
				solvedProblems[0].solvedAt
			).toDateString();
			const daysSinceLastSolved = Math.floor(
				(new Date(today) - new Date(lastSolvedDate)) / (1000 * 60 * 60 * 24)
			);
...
// @route   POST /api/problems/:id/request-approval
// @desc    Request approval to make problem global
// @access  Private
router.post("/:id/request-approval", auth, async (req, res) => {
	try {
		...
		if (problem.approvalStatus === "approved") {
			return res.status(400).json({ message: "Problem already approved" });
		}

		problem.approvalStatus = "pending";
		problem.approvalRequestedAt = new Date();
		await problem.save();

		res.json({ message: "Approval request submitted successfully", problem });
	} catch (error) {
