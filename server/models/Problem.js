...
const problemSchema = new mongoose.Schema(
	{
		...
		// Approval workflow for user-created problems
		approvalStatus: {
			type: String,
...
problemSchema.pre("save", function (next) {
	// Set solvedAt whenever status is changed to solved
	if (this.isModified("status")) {
		if (this.status === "solved") {
			this.solvedAt = new Date();
		} else if (this.status !== "solved" && this.solvedAt) {
			// Clear solvedAt if status changes from solved to something else
