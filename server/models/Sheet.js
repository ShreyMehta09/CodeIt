...
const sheetSchema = new mongoose.Schema(
	{
		...
		// Approval workflow for user-created public sheets
		approvalStatus: {
			type: String,
		...
		problems: [
			{
				...
				status: {
					type: String,
					enum: ["todo", "attempted", "solved", "review"],
					default: "todo",
				},
			},
		],
...
sheetSchema.index({ approvalStatus: 1 });
sheetSchema.index({ isGlobal: 1 });
...
sheetSchema.methods.updateProgress = async function () {
	...
	// Get all problems in this sheet
	const problemsWithDetails = await Problem.find({
		_id: { $in: this.problems.map((p) => p.problemId) },
	});

	this.totalProblems = this.problems.length;

	// Count solved/attempted/review from the problems array (user-specific status)
	this.solvedProblems = this.problems.filter(
		(p) => p.status === "solved"
	).length;

	// Update difficulty distribution
	this.difficultyDistribution = {
		easy: problemsWithDetails.filter((p) => p.difficulty === "easy").length,
		medium: problemsWithDetails.filter((p) => p.difficulty === "medium").length,
		hard: problemsWithDetails.filter((p) => p.difficulty === "hard").length,
	};
...
sheetSchema.statics.getTemplates = async function () {
	return this.find({ isTemplate: true }).select(
		"name description category totalProblems difficultyDistribution templateSource"
	);
};
