...
const Profile = () => {
	...
	// Calculate combined rating from all platforms
	const calculateCombinedRating = (platformData) => {
		...
		// Normalize and weight ratings from each platform
		if (platformData.leetcode?.rating) {
			// LeetCode rating typically ranges from 1200-3000+, normalize to match other platforms
			const normalizedLeetcode = (platformData.leetcode.rating / 3000) * 3000;
			totalRating += normalizedLeetcode * weights.leetcode;
			platformCount++;
		}

		if (platformData.codeforces?.rating) {
			// Codeforces rating typically ranges from 0-3500+
			totalRating += platformData.codeforces.rating * weights.codeforces;
			platformCount++;
		}

		if (platformData.codechef?.rating) {
			// CodeChef rating typically ranges from 0-3500+
			totalRating += platformData.codechef.rating * weights.codechef;
			platformCount++;
		}

		// Adjust weights if some platforms are missing
		if (platformCount === 0) return 0;

		const adjustedRating =
			platformCount < 3 ? totalRating * (3 / platformCount) : totalRating;
		return Math.round(adjustedRating);
	};
	...
	useEffect(() => {
		const loadCachedData = async () => {
			try {
				...
				if (
					response.data.cachedPlatformData &&
					Object.keys(response.data.cachedPlatformData).length > 0
				) {
					...
					const formattedData = {
						...
						totalProblems: Object.values(
							response.data.cachedPlatformData
						).reduce((sum, platform) => sum + (platform.totalSolved || 0), 0),
						lastSyncTime:
							response.data.platforms?.leetcode?.lastSynced ||
						...
						isDemo: false,
						summary: {
							platformsConnected: Object.keys(response.data.cachedPlatformData)
								.length,
							totalProblems: Object.values(
								response.data.cachedPlatformData
							).reduce((sum, platform) => sum + (platform.totalSolved || 0), 0),
	...
	const handleSync = async () => {
		try {
			...
			setSyncError(
				error.response?.data?.message || error.message || "Failed to sync data"
			);
		} finally {
		...
		try {
			...
			if (navigator.share) {
				await navigator.share({
					title: `${user.name}'s CodeIt Profile`,
					text: `Check out ${user.name}'s coding profile on CodeIt!`,
					url: profileUrl,
				});
			} else {
	...
	const renderRatingGraph = (platformData, platformName) => {
		if (
			...
			return (
				<div className="text-sm text-gray-500">No rating history available</div>
			);
		}

		// Sort rating history by timestamp for accurate chronological display
		const sortedHistory = [...platformData.ratingHistory].sort(
	...
	const combinedRating = calculateCombinedRating(syncData?.platformData);
	const ratingTier = getRatingTier(combinedRating);
	...
	return (
		<div className="space-y-6">
							...
							<div className="flex flex-col sm:flex-row sm:items-start sm:justify-between">
								...
								<div className="flex space-x-2 mt-4 sm:mt-0">
									...
									<div className="relative">
										...
										{showShareTooltip && (
											<div className="absolute top-full mt-2 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-xs px-3 py-2 rounded shadow-lg whitespace-nowrap z-10">
			...
			{syncData &&
				syncData.platformData &&
				Object.keys(syncData.platformData).length > 0 &&
				combinedRating > 0 && (
					<div className={`card ${ratingTier.borderColor} border-2`}>
						<div className="card-content">
							<div className="flex items-center justify-between flex-wrap gap-4">
								<div className="flex items-center space-x-4">
									<div className={`${ratingTier.bgColor} p-4 rounded-full`}>
										<TrendingUp className={`w-8 h-8 ${ratingTier.color}`} />
									</div>
									<div>
										...
										<div className="flex items-baseline space-x-2">
											<span
												className={`text-4xl font-bold ${ratingTier.color}`}
											>
												{combinedRating}
											</span>
											<span
												className={`text-lg font-semibold ${ratingTier.color}`}
											>
												{ratingTier.tier}
											</span>
										</div>
										<p className="text-xs text-gray-500 mt-1">
											Weighted average from all connected platforms
										</p>
									</div>
								</div>

								<div className="flex flex-col space-y-2 min-w-[200px]">
									{syncData.platformData.leetcode?.rating && (
									...
									{syncData.platformData.codeforces?.rating && (
										<div className="flex items-center justify-between space-x-3 text-sm">
									...
									{syncData.platformData.codechef?.rating && (
										<div className="flex items-center justify-between space-x-3 text-sm">
								...
								<details className="text-xs text-gray-600">
									...
									<div className="mt-2 p-3 bg-gray-50 rounded">
										<p className="mb-2 font-mono text-xs">
											Combined Rating = (LeetCode × 35%) + (Codeforces × 40%) +
											(CodeChef × 25%)
										</p>
										<p className="text-xs text-gray-500">
											• LeetCode ratings are normalized to match other
											platforms' scale
											<br />
											• Weights are adjusted proportionally if platforms are
											missing
											<br />• <strong>Rating tiers:</strong> Newbie (0-799),
											Pupil (800-1199), Apprentice (1200-1499), Specialist
											(1500-1799), Expert (1800-2099), Master (2100-2399),
											Grandmaster (2400+)
										</p>
									</div>
								</details>
					...
					<div className="card-content">
						<div className="text-green-700 dark:text-green-400 font-medium">
							...
							<span className="text-green-600 dark:text-green-400 text-sm ml-2">
								Total Problems: {syncData.totalProblems} | Last Sync:{" "}
						...
						{syncData.isDemo && (
							<div className="text-blue-600 dark:text-blue-400 text-sm mt-2 bg-blue-50 dark:bg-blue-900/30 p-2 rounded">
								<strong>Demo Mode:</strong> Connect your real platform accounts
								in Settings to see your actual data.
							</div>
						)}
					...
					<div className="card">
						<div className="card-header">
							...
							<p className="card-description">
								Your rating progression across platforms
							</p>
						</div>
						<div className="card-content">
							<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
								{Object.entries(syncData.platformData).map(
									([platform, data]) => (
															...
															<tbody>
																{[...data.ratingHistory]
																	...
																	.map((contest, index, sortedContests) => {
																		...
																		const ratingChange = nextContest
																			? contest.rating - nextContest.rating
																			: null;
																		...
																		return (
																			...
																			>
																				<td
																					className="px-3 py-2 text-gray-800 truncate max-w-32"
											...
											style={{
												width: `${
													syncData.platformData.leetcode.totalSolved > 0
														? (syncData.platformData.leetcode.easy /
									...
									<div className="mt-2 bg-warning-100 dark:bg-warning-900/30 rounded-full h-2">
										<div
											...
											style={{
												width: `${
													syncData.platformData.leetcode.totalSolved > 0
														? (syncData.platformData.leetcode.medium /
																syncData.platformData.leetcode.totalSolved) *
														  100
														: 0
												}%`,
									...
									<div className="mt-2 bg-danger-100 dark:bg-danger-900/30 rounded-full h-2">
										<div
											...
											style={{
												width: `${
													syncData.platformData.leetcode.totalSolved > 0
														? (syncData.platformData.leetcode.hard /
																syncData.platformData.leetcode.totalSolved) *
														  100
														: 0
												}%`,
									...
									<div className="text-center p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
										...
										<div className="text-xl font-bold text-blue-600 dark:text-blue-400 mt-1">
											{syncData.platformData.leetcode.globalRanking
												...
												? `#${syncData.platformData.leetcode.ranking.toLocaleString()}`
												: "N/A"}
										</div>
									</div>
									<div className="text-center p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
